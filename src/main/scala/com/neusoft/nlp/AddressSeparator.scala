...
class AddressSeparator(val similarity_threshold: Double = 0.5) extends Serializable {
  ...
  /**
    ...
    * @return 沈阳市铁西区滑翔小区
    */
  def removeDuplicate(address: String): String = {
    if (address == null) ""
    else {
  ...
  /**
    ...
    * @return
    */
  def findSimilarityAddress(address: String, candidateAddressList: Traversable[TreeNode]): Option[TreeNode] = {
    // 无地址信息
    if (address == null || address == "") None
    // 由于地区数据缺失，没有候选节点信息
    if (candidateAddressList.isEmpty) None
    else {
  ...
  def splitAddress(address: String): (String, String, String, String) = {
    ...
    // 查找所属城市
    val belongsCityInfo: Option[TreeNode] = findSimilarityAddress(addr, searchMap.values)
    ...
    if (belongsCityInfo.isDefined) {
      ...
      // 判断是否是直辖市
      if (MUNICIPALITY.contains(belongsCityNode.addressName)) {
        // 直辖市即省份
    ...
    // 若无所属城市信息，查找省节点信息
      ...
      val belongsProvinceInfo = findSimilarityAddress(addr, tree.childNode)
